...
PREDICATES
	...
	findByPhoneNumber(surname, phoneNumber, brand, price).
	findByPhoneNumber2(phoneNumber, brand).
	
	findBySurname(surname, city, street, bank, phoneNumber).
...
CLAUSES
	...
	% 1) a
	findByPhoneNumber(Surname, PhoneNumber, Brand, Price) :-
		phonebook(Surname, PhoneNumber, _),
		car(Surname, _, Brand, _, Price).
		
	% 1) b
	findByPhoneNumber2(PhoneNumber, Brand) :-
		findByPhoneNumber(_, PhoneNumber, Brand, _).
	
	% 2)
	findBySurname(Surname, City, Street, Bank, PhoneNumber) :-
		phonebook(Surname, PhoneNumber, address(City, Street, _, _)),
		bankDepositor(Surname, City, Bank, _, _).
		
GOAL
	%findByPhoneNumber(Surname, "8 (777) 123-23-32", Brand, Price).
	
	%findByPhoneNumber2("8 (777) 123-23-32", Brand).
	
	findBySurname("Ivanov", "Moscow", Street, Bank, PhoneNumber).
